...
async def send(
    ...
    **Specifying both parameters will lead to an exception**.
    ...
    if file is not None and files is not None:
        raise Error("cannot pass both file and files parameter to send()")

    if file is not None:
        if not isinstance(file, File):
            raise Error("file parameter must be File")
        ...
        if len(files) > 10:
            raise Error("files parameter must be a list of up to 10 elements")
        elif not all(isinstance(file, File) for file in files):
            raise Error("files parameter must be a list of File")
